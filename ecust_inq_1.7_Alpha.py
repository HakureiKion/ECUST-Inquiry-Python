...
1.7 ALPHA:
    加入课程表查询第一版代码
    已知问题：
        ……获取不到课表，返回中包含“查询中”提示信息
        一个课程时间内有多门课程时会从第二门课程开始连续无间隔输出
'''
...
##    print(data)
    ...
    try:
        ...
        index = 0
        for td in tds:
            cname = td.find('div',attrs={'class':'kbcontent'})
            tname = td.find('font',attrs={'title':'老师'})
            prd = td.find('font',attrs={'title':'周次(节次)'})
            loc = td.find('font',attrs={'title':'教室'})
            if tname == None:
                continue
            cname = cname.getText()
            tname = tname.getText()
            prd = prd.getText()
            loc = loc.getText()
            dblcourse = cname.find('-----------------')
            # 如果一格里有两门课，找到字符串分隔位置
            if dblcourse >= 0:
                cname2, tname2, prd2, loc2 = cname, tname, prd, loc
                cname2 = cname2[:dblcourse].rstrip(loc).rstrip(prd).rstrip(tname)
                print(cname2)
                print(tname2)
                print(prd2)
                print(loc2)
                #前一门课程
                cname1 = cname.lstrip(cname2).lstrip(tname2).lstrip(prd2).lstrip(loc).lstrip('-----------------')
                print(cname1)
            else:
                cname1 = cname.rstrip(loc).rstrip(prd).rstrip(tname)
                print(cname1)
                print(tname)
                print(prd)
                print(loc)
